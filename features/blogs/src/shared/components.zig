...
const InlineScriptRenderer = struct {
    content: ?[]const u8, pub fn render(self: @This(), writer: anytype) !void {
        if (self.content) |value| {
            // Write script tag with raw content (no HTML escaping)
    ...
    pub fn render(self: HomepageBodyDynamic, writer: anytype) !void {
        const body_el = body(Attrs{ .class = self.config.class }, .{
            ...
            div(Attrs{ .id = "main-content" }, .{
                ...
                ResumeSectionDynamic.init(self.config.resume_section), PortfolioSectionDynamic.init(self.config.portfolio),
                BlogSectionDynamic.init(self.config.blog), PlaygroundSectionDynamic.init(self.config.playground),
            }),
