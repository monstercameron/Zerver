...
pub const Attrs = struct {
    // Global attributes
    id: ?[]const u8 = null, class: ?[]const u8 = null,
    style: ?[]const u8 = null,
    title: ?[]const u8 = null,
    lang: ?[]const u8 = null,
...
pub fn Element(
    comptime tag: []const u8, comptime AttrType: type,
    comptime Children: type,
) type {
    return struct {
        const Self = @This();
        attrs: AttrType, children: Children,
        ...
        inline fn renderAttr(writer: anytype, name: []const u8, value: anytype) !void {
            // Convert underscores to hyphens for HTML attributes (e.g., hx_get -> hx-get)
            var attr_name_buf: [128]u8 = undefined;
            const attr_name = blk: {
                var idx: usize = 0;
                for (name) |c| {
                    if (idx >= attr_name_buf.len) break :blk name; // Fallback if name too long
                    attr_name_buf[idx] = if (c == '_') '-' else c;
                    idx += 1;
                }
                break :blk attr_name_buf[0..idx];
            };

            const ValueType = @TypeOf(value);
            switch (@typeInfo(ValueType)) {
                .bool => if (value) try writer.print(" {s}", .{attr_name}),
                .int, .comptime_int, .float, .comptime_float => try writer.print(" {s}=\"{}\"", .{ attr_name, value }),
                .optional => {
                    if (value) |some| {
