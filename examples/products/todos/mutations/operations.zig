...
pub fn mutation_create_todo(ctx: *zerver.CtxBase) !zerver.Decision {
    ...
    const new_id = std.fmt.bufPrint(&id_buf, "todo_{d}", .{time_util.timestamp()}) catch unreachable;
...
pub fn mutation_update_todo(ctx: *zerver.CtxBase) !zerver.Decision {
    ...
    time_util.sleep(read_latency * 1_000_000);
...
pub fn render_created(_: *zerver.CtxBase) !zerver.Decision {
    ...
    return zerver.done(.{
        .status = 201, .body = "{\"id\":\"generated_id\",\"created_at\":\"2025-01-01T00:00:00Z\"}",
    });
}
...
pub fn render_updated(ctx: *zerver.CtxBase) !zerver.Decision {
    ...
    return zerver.done(.{
        .status = 200, .body = "{\"id\":\"updated_id\",\"updated_at\":\"2025-01-01T00:00:00Z\"}",
    });
}
